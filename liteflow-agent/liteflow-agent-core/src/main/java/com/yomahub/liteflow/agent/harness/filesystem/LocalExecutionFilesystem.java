...
public final class LocalExecutionFilesystem extends LocalFilesystemWithShell {
    ...
    private Map<String, byte[]> materialize(RuntimeContext context, Path directory) throws IOException {
        ...
        // A local deployment may point both roots at the same directory. Copying the
        // source onto itself truncates live files and can delete newly created output
        // as if it were a stale cache entry. There is no cache to hydrate in this case.
        if (usesAuthoritativeDirectory()) return Map.of();
    ...
    private void synchronize(RuntimeContext context, Path directory, Map<String, byte[]> before) throws IOException {
        ...
        // Keep the output-tree validation above, but local changes are already durable.
        if (usesAuthoritativeDirectory()) return;
    ...
    private boolean usesAuthoritativeDirectory() throws IOException {
        return files instanceof GuardedLocalFilesystem local
                && Files.isSameFile(local.getCwd(), getCwd());
    }
