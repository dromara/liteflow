...
/**
 * WHEN 并发分支 Agent A：覆写 agentKey 含 requestId，确保和分支 B 拥有独立的 Session 锁，
 * 这样才能真正并发执行而不互相串行。
 */
@Component("parallelAgentA")
public class ParallelAgentACmp extends OfflineAgentComponent {
    ...
    protected void handleReply(Msg reply, com.yomahub.liteflow.agent.context.LiteFlowAgentContext context) {
        ...
        SEEN_REPLY.set(text);
        context.getSlot().setOutput("parallelAgentA", text);
    }
}
