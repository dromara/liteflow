...
/**
 * Kimi（OpenAI 兼容预设）基础连通性场景测试。
 * 需要 {@code LITEFLOW_AGENT_TEST_KIMI_API_KEY}，缺失即跳过。
 */
@TestPropertySource("classpath:/platform/kimi/application.properties")
@SpringBootTest(classes = KimiPlatformTest.class)
@EnableAutoConfiguration
@ComponentScan("com.yomahub.liteflow.test.agent.platform.kimi")
public class KimiPlatformTest extends BaseAgentLiveTest {
    ...
    public void ensureCredential() {
        LiveTestSupport.applyCompatiblePresetOrSkip(
                liteflowConfig, "kimi", LiveTestEnv.KIMI_API_KEY, "KimiPlatformTest");
    }

    @Test
    public void testKimiConnectivity() {
        LiteflowResponse response = flowExecutor.execute2Resp("kimiPlatformChain", PROMPT);
