...
public class DockerSandboxLiveTest extends RealAgentTestBase {
    ...
    public void modelRunsCommandInsideRealDockerSandbox() {
        var events = new java.util.concurrent.CopyOnWriteArrayList<FlowEvent>();
        LiteflowResponse response = flowExecutor.execute2Resp("realDockerSandboxChain",
                "请使用 execute 工具执行命令 echo DOCKER-SANDBOX-OK，"
                        + "然后把命令输出原样告诉我。", ExecuteOption.of().eventListener(events::add));
        ...
        Assertions.assertTrue(events.stream().anyMatch(event ->
                "agent.tool.call.start".equals(event.getType())
                        && event.getData() instanceof com.yomahub.liteflow.agent.event.AgentFlowEventData dataEvent
                        && dataEvent.event() instanceof io.agentscope.core.event.ToolCallStartEvent tool
                        && "execute".equals(tool.getToolCallName())));
        Assertions.assertTrue(events.stream().anyMatch(event ->
                "agent.tool.result.end".equals(event.getType())
                        && event.getData() instanceof com.yomahub.liteflow.agent.event.AgentFlowEventData dataEvent
                        && dataEvent.event() instanceof io.agentscope.core.event.ToolResultEndEvent tool
                        && "execute".equals(tool.getToolCallName())),
                "the model must actually execute the command, not just repeat the prompt");
    }
