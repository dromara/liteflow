 ...
 * 为 guide §6.5 的 A2A 客户端测试提供真实远程端点。
 */
final class MinimalA2aServer implements AutoCloseable {
    ...
    final List<String> receivedAuthorization = new CopyOnWriteArrayList<>();
    volatile int rejectionStatus = 401;
    ...
    private void handle(com.sun.net.httpserver.HttpExchange exchange) throws IOException {
        try {
            ...
            receivedAuthorization.add(String.valueOf(authorization));
            if (!"Bearer protocol-token".equals(authorization)) {
                respond(exchange, rejectionStatus, "{\"error\":\"unauthorized\"}");
                return;
            }
