...
import com.yomahub.liteflow.test.agent.feature.cmp.MaxIterationsAgentCmp;
 ...
 *
 * <p>断言通过组件注册的 Hook 读取 {@code ReActAgent.getMaxIters()}：默认值（{@code liteflow.agent.defaults.max-iterations}=6）
 * 与组件覆写值（{@link MaxIterationsAgentCmp#OVERRIDDEN_MAX_ITERS}=7）不同，确认覆写生效。
 */
...
public class MaxIterationsTest extends BaseAgentLiveTest {
    ...
    public void resetProbe() {
        ...
        LiveTestSupport.ensureCompatibleCustomCredentialOrSkip(liteflowConfig, "MaxIterationsTest");
    }
