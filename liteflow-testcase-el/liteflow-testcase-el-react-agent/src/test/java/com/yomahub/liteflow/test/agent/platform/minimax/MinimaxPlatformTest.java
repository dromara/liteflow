...
/**
 * Minimax（OpenAI 兼容预设）基础连通性场景测试。
 * 需要 {@code LITEFLOW_AGENT_TEST_MINIMAX_API_KEY}，缺失即跳过。
 */
@TestPropertySource("classpath:/platform/minimax/application.properties")
@SpringBootTest(classes = MinimaxPlatformTest.class)
@EnableAutoConfiguration
@ComponentScan("com.yomahub.liteflow.test.agent.platform.minimax")
public class MinimaxPlatformTest extends BaseAgentLiveTest {
    ...
    public void ensureCredential() {
        LiveTestSupport.applyCompatiblePresetOrSkip(
                liteflowConfig, "minimax", LiveTestEnv.MINIMAX_API_KEY, "MinimaxPlatformTest");
    }

    @Test
    public void testMinimaxConnectivity() {
        LiteflowResponse response = flowExecutor.execute2Resp("minimaxPlatformChain", PROMPT);
