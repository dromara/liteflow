...
import com.yomahub.liteflow.test.agent.feature.cmp.ShellToolsAgentCmp;
 ...
 * 每个测试都在同一条 THEN 链路中调用 Agent，但 BeforeEach 调整全局
 * {@code liteflow.agent.shell.mode}，验证 Toolkit 注册和命令执行行为。
 */
...
public class ShellToolModesFeatureTest extends BaseAgentLiveTest {
    ...
    public void reset() {
        ...
        LiveTestSupport.ensureCompatibleCustomCredentialOrSkip(liteflowConfig, "ShellToolModesFeatureTest");
    }
    ...
    public void testDisabledModeSkipsToolRegistrationAndDeniesExecution() {
        ...
        // 直接调用 ManagedShellCommandTool 时，DISABLED 模式应返回拒绝消息。
    ...
    public void testWhitelistModeOnlyAllowsListedCommands() {
        ...
        // pwd 在白名单中，应执行成功，结果为 workspace 路径。
        String pwd = ShellToolsAgentCmp.PWD_OUTPUT.get();
        Assertions.assertNotNull(pwd);
        Assertions.assertEquals(ShellToolsAgentCmp.WORKSPACE.get(), pwd.trim());
