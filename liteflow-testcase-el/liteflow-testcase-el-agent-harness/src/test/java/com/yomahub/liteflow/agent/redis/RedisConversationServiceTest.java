...
class RedisConversationServiceTest {
    ...
    void metadataPagingMessagesAndTombstonesRoundTripThroughRedisStore() {
        ...
        try (var service = new AgentConversationService(config,
            ...
            assertFalse(client.values.isEmpty());
            assertTrue(client.values.keySet().stream().allMatch(key -> key.startsWith("agentscope:session:")),
                    "blank key-prefix must retain the documented effective default");
        }
