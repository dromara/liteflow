# liteflow-agent Skills Support Design

## Goal

Add first-class skills support to `liteflow-agent` by reusing the agent-scope `SkillBox` model and following the skill design pattern from `/Users/bryan31/AiProject/beast-agent-service`.

The feature lets LiteFlow agents load skills from a configured filesystem repository, optionally restrict each agent component to a declared allow-list of skills, and bind skill-specific Java tools through `SKILL.md` frontmatter.
...
`liteflow-agent` currently builds a `Toolkit` inside `AgentComponent#buildAgent()` and registers:
...
`beast-agent-service` uses this pattern:
...
Add these extension points to `AgentComponent`:
...
```text
liteflow-agent/liteflow-agent-core/src/main/java/com/yomahub/liteflow/agent/skill/
```
...
This avoids re-scanning inside hooks and keeps `AgentComponent` simple.

## AgentComponent Integration

Update `AgentComponent#buildAgent()`:
...
Expose used skills through `AgentComponent` with:
...
- `liteflow-agent` already owns workspace file access through `WorkspaceFileTools`.
...
Add tests in `liteflow-testcase-el/liteflow-testcase-el-agent`.
...
```text
liteflow-testcase-el/liteflow-testcase-el-agent/src/test/resources/agent/skills/
```
...
- Use `AgentComponent#skills()` to declare an allow-list.
...
- Existing `liteflow-agent` tests pass with skills disabled.
