# AgentComponent 模型声明 API 重设计

- 日期: 2026-04-29
- 范围: liteflow-agent-core 与四个 provider 子模块
- 状态: 已对齐，待实现

## 1. 背景

当前 `AgentComponent` 暴露 `protected abstract Model buildModel(AgentContext ctx)`，子类必须自行：
...
位置：`liteflow-agent-core`，包 `com.yomahub.liteflow.agent.model`。
...
public class DeepSeekAgentCmp extends AgentComponent {
    ...
    @Override protected String systemPrompt(AgentContext ctx) { ... }
...
- **新增**：`liteflow-agent-core` 中的 `com.yomahub.liteflow.agent.model.ModelSpec`。
- **改动**：`liteflow-agent-core/AgentComponent`——`buildModel` 不再 `abstract`，新增 `abstract model(ctx)`。
...
- **测试**：`liteflow-testcase-el-agent` 增补针对新 API 的用例。
