...
**日期**: 2026-05-09
**作用范围**: `liteflow-core` (Slot)、`liteflow-agent-core`、`liteflow-testcase-el-agent`
**类型**: 破坏式重构
...
当前 `AgentComponent` 的 hook 方法签名混乱、语义不清：
...
protected List<Hook> hooks(AgentContext ctx);
...
**问题 A：签名不统一**
同一抽象类的 hook，有的传 `AgentContext`，有的传 `Slot`，有的无参，使用者很难记忆。

**问题 B：构建期与运行期上下文混合**
- `tools / hooks / systemPrompt / model` 是**构建期 hook**——agent 按 `(conversationId, agentKey)` 缓存（见 `AgentComponent.java:221-225`），这些方法只在 agent 首次构建时调用一次。
- `userPrompt / handleReply` 是**运行期 hook**——每次 `process()` 都调用。
- 但当前两类方法都接收同一个 `AgentContext`。该 ctx 内部持有 `Slot`，而 Slot 是**每次执行**才有效的对象。
...
`AgentContext` 语义上是"本次调用的上下文"，本应与 `Slot`（per-invocation 容器）同生命周期；但当前它被作为参数传给 agent 构建函数，又因为 agent 缓存导致 ctx 被永久捕获，与"per-invocation"语义矛盾。
...
- 命名前缀建议使用插件特定前缀（如 `_agent_ctx_`）避免与 Slot 已有的内部 key（`_request_`/`_response_`/`_chain_id` 等）冲突。

### 2.2 AgentComponent 全面无参化

**文件**: `liteflow-agent/liteflow-agent-core/src/main/java/com/yomahub/liteflow/agent/component/AgentComponent.java`
...
### 2.3 AgentContext 保持不变

`AgentContext` 类本身**不改动**：依然包含 `slot / conversationId / agentKey / workspaceDir` 四个字段。改动的是**它如何被访问**——从"参数传递"改为"通过 slot attachment 间接获取"。

> 备选方案：移除 `AgentContext.getSlot()`，强制用户走 `ctx().getSlot()` 而非缓存 ctx 引用。但这会让现有用户代码（如测试中的 StubModel）改动量更大。本次重构**保留 `getSlot()`**，在文档中明确警告"勿在跨 invocation 缓存的对象（工具/Hook 等）中持有 ctx 引用"。
...
class BadModel {
    ...
    BadModel(AgentContext ctx) { this.ctx = ctx; }
    void use() { ctx.getSlot().setData(...); }   // stale slot
}

// 正确：捕获组件引用，运行时动态取 ctx
class GoodModel {
    private final AgentComponent comp;
    GoodModel(AgentComponent comp) { this.comp = comp; }
...
- `AgentELChainTest.java`
- `AgentSessionTest.java`
- `AgentWorkspaceTest.java`
- `AgentConversationContinuityTest.java`（新文件）
- `AgentMultiAgentChainTest.java`（新文件）
...
- 不在 NodeComponent 层提取通用 per-invocation context 机制（用户已确认作用域控制在 agent 模块）。
...
2. agent-core: 改 `AgentComponent`——加 `ctx()` 访问器、改 `process()` 绑定/解绑、所有 hook 改无参
3. 同步更新 `AgentContext` 的 Javadoc（警告勿持有 ctx 引用）
4. 改造 `StubAgentCmp` 和现有 5 个测试文件
5. 新增 3 个专项测试（陈旧 ctx 防护 / WHEN 并发 / 越界调用 ctx()）
6. 跑全量测试：`mvn test -pl liteflow-testcase-el/liteflow-testcase-el-agent`
